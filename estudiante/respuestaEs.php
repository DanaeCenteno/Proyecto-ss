...
iniciarSesion();
requiereLogin(); // Cualquier usuario logueado puede ver respuestas (estudiante o profesor)
            ...
            <div class="col-lg-8">
                ...
                <div class="ask-card pregunta-original mb-4">
                    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                        ...
                        <span class="badge px-3 py-2 status-<?= $pregunta['estado'] ?>" style="border-radius:20px;font-size:12px;">
                    ...
                    <div class="d-flex gap-2 mt-3 flex-wrap">
                        <select class="form-select form-select-sm" style="max-width:160px;"
                            ...
                            <option value="abierta"  <?= $pregunta['estado']==='abierta'  ? 'selected':'' ?>>Abierta</option>
                            <option value="resuelta" <?= $pregunta['estado']==='resuelta' ? 'selected':'' ?>>Resuelta</option>
                            <option value="cerrada"  <?= $pregunta['estado']==='cerrada'  ? 'selected':'' ?>>Cerrada</option>
                        </select>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <?php else: ?>
                <h6 class="fw-semibold mb-3"><?= $totalRespuestas ?> respuesta<?= $totalRespuestas!==1?'s':'' ?></h6>
                ...
                <div class="ask-card mb-3 <?= $r['es_solucion'] ? 'border border-success' : '' ?>">
                    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                        ...
                        <!-- Eliminar (solo el autor de la respuesta) -->
                        <?php if ($uid === (int)$r['usuario_id']): ?>
                        <button class="btn btn-sm btn-outline-danger" onclick="eliminarRespuesta(<?= $r['id'] ?>)">
                ...
                <div class="ask-card" id="form-respuesta">
                    <div class="mb-4">
                        ...
                        <span class="invalid-msg" id="err-resp-contenido">Escribe tu respuesta antes de publicar.</span>
                    ...
                    <div class="ask-actions">
                        ...
                        <div class="actions-right">
                            ...
                            <button class="btn btn-primary px-4 fw-semibold" type="button" onclick="publicarRespuesta()">
...
<div class="preview-backdrop" id="preview-backdrop-resp" onclick="backdropClickResp(event)">
    <div class="preview-modal" id="preview-modal-resp">
        ...
        <div class="preview-body"><div id="preview-resp-wrap"></div></div>
...
async function cambiarEstado(preguntaId, nuevoEstado) {
    ...
    try {
        ...
        mostrarToast(data.ok ? 'Estado actualizado.' : (data.msg || 'Error.'), data.ok ? 'success' : 'error');
...
async function eliminarRespuesta(respuestaId) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.ok) { mostrarToast('Respuesta eliminada.', 'success'); setTimeout(() => location.reload(), 1000); }
