...
<body>
        ...
        <ul class="nav align-items-center gap-1 mb-0">
            ...
            <li>
                <a href="perfil.php?uid=<?= $uid ?>" class="nav-link px-2">
                    ...
                    <img src="<?= htmlspecialchars($avatarUsuario) ?>" class="user-avatar-img" alt="Avatar"
            ...
            <form method="GET" action="foroEs.php">
                <div class="d-flex flex-wrap gap-2 align-items-center">
                    <input type="text" name="q" class="form-control" style="max-width:280px;"
                        placeholder="Buscar por título o lenguaje..." value="<?= htmlspecialchars($busqueda) ?>">
                    ...
                    <select name="lang" class="form-select" style="max-width:160px;">
                        ...
                        <?php
                    $langs = ['python','javascript','typescript','java','csharp','cpp','go','rust','php','ruby','swift','kotlin','sql','bash', 'HTML', 'CSS'];
                    foreach ($langs as $l):
                    ?>
                        <option value="<?= $l ?>" <?= $filtroLang === $l ? 'selected' : '' ?>><?= ucfirst($l) ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
    ...
    <div class="btn-ask text-end container mt-3">
        <a href="askEs.php" class="btn btn-primary fw-semibold px-4 py-2"
    ...
    <div class="foro-list mt-3">
        <div class="container">
            ...
            $langs = $p['lenguajes']
                ...
                : [];
        ?>
            <div class="pregunta-card p-4 mb-3 shadow-sm bg-white"
                style="border-radius:12px; border-left: 5px solid #219EBC;">
                <div class="d-flex align-items-center gap-3">
                    <?php
                    $avAutor = $p['autor_avatar'] ?? '';
                    if (!empty($avAutor) && !str_starts_with($avAutor, 'http')) {
                        $rutaAv = ltrim($avAutor, '/');
                        $avAutor = file_exists(__DIR__ . '/../' . $rutaAv) ? ('../' . $rutaAv) : '';
                    }
                ?>
                <?php if (!empty($avAutor)): ?>
                    <img src="<?= htmlspecialchars($avAutor) ?>"
                        alt="<?= htmlspecialchars($p['autor_nombre']) ?>" class="autor-avatar-img"
                        style="width:40px; height:40px; border-radius:50%; object-fit:cover;">
                    <?php else: ?>
                    <div class="autor-avatar-initials d-flex align-items-center justify-content-center bg-secondary text-white fw-bold rounded-circle"
                    ...
                    <span class="badge text-capitalize px-3 py-2 status-<?= $p['estado'] ?>"
                ...
                <div class="mt-3">
                    <a href="respuestaEs.php?id=<?= $p['id'] ?>"
                        class="pregunta-titulo text-decoration-none fw-bold fs-5 text-dark d-block">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 pregunta-footer">
                    ...
                    <a href="respuestaEs.php?id=<?= $p['id'] ?>" class="btn btn-sm btn-outline-primary px-3 fw-medium"
    ...
    <footer>
        <div id="div-footer">
            ...
            <div style="display:flex;gap:24px;">
                ...
                <a href="foroEs.php" style="font-size:13px;color:rgba(255,255,255,.5);text-decoration:none;">Foro</a>
            </div>
        </div>
    </footer>
