...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Sans:wght@300;400;500;600&family=Poppins:wght@300;400;500;600;700&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        .students-table tbody tr:hover { background: rgba(30, 73, 120, .03); }
        ...
        .modal-detalle .modal-body { padding: 24px; }
        ...
        .leccion-check.ok   { background: rgba(46,204,113,.15); color: #27ae60; }
        .leccion-check.pend { background: rgba(220,140,30,.12); color: #DC8C1E; }
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="perfil.php" class="nav-link-item">
                ...
                <div class="filters-bar">
                    ...
                    <div class="filter-group" style="min-width:auto; flex:none;">
                        ...
                        <select name="orden" class="filter-input" style="min-width:140px;">
                            ...
                            <option value="progreso" <?= $filtroOrden == 'progreso' ? 'selected' : '' ?>>Mayor progreso</option>
                        </select>
                    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="alumno-info">
                                ...
                                <div>
                                    <div class="alumno-nombre"><?= htmlspecialchars($e['alumno_nombre']) ?></div>
                                    <div class="alumno-correo"><?= htmlspecialchars($e['alumno_correo']) ?></div>
                                </div>
                            </div>
                        </td>
...
<div class="modal fade modal-detalle" id="modalDetalle" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content" style="border-radius:16px; border: 1px solid var(--border); box-shadow: 0 20px 60px rgba(30,73,120,.15);">
...
function abrirDetalle(alumnoId, cursoId) {
    ...
    fetch(`../administrador/api/estudiantes/detalle_estudiante.php?alumno_id=${alumnoId}&curso_id=${cursoId}`)
        .then(r => r.json())
        .then(data => {
            if (!data.success) {
                modalBody.innerHTML = `<p class="text-danger text-center py-3">${data.error ?? 'Error al cargar.'}</p>`;
                return;
            }
            renderDetalle(data);
        })
        .catch(() => {
            modalBody.innerHTML = `<p class="text-danger text-center py-3">Error de conexión.</p>`;
        });
}

function renderDetalle(data) {
    const { alumno, curso, modulos } = data;
    ...
    const iniciales = alumno.nombre.trim().split(' ')
        .slice(0, 2).map(w => w[0]?.toUpperCase() ?? '').join('');

    const avatarHtml = alumno.avatar
        ? `<img src="${alumno.avatar}" alt="" style="width:100%;height:100%;object-fit:cover;">`
        : iniciales;

    const iconoTipo = { video: 'bi-play-circle', practica: 'bi-code-slash', texto: 'bi-file-text', documento: 'bi-file-earmark' };
    ...
    const modulosHtml = modulos.map(mod => {
        const leccionesHtml = mod.lecciones.map(l => {
            ...
            <div class="leccion-row">
                <div class="leccion-check pend">
                    <i class="bi ${icono}"></i>
                </div>
                <span style="flex:1; color: var(--text, #2c3e50); font-size:13px;">
                    ${escHtml(l.titulo)}
                </span>
                <span style="font-size:10px;color:#4A90B9;text-transform:uppercase;letter-spacing:.05em;font-weight:600;">${l.tipo}</span>
            </div>
        `}).join('');
        ...
        return `
            <div style="margin-bottom:18px;">
                <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#4A90B9;margin-bottom:8px;">
                    <i class="bi bi-collection" style="margin-right:5px;"></i>${escHtml(mod.titulo)}
                </div>
                ${leccionesHtml || '<p style="font-size:12px;color:#999;padding:6px 0;">Sin lecciones</p>'}
            </div>
        `;
    }).join('') || '<p style="font-size:13px;color:#999;">Este curso no tiene módulos aún.</p>';

    modalBody.innerHTML = `
        <!-- Cabecera alumno -->
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
            <div class="detalle-avatar">${avatarHtml}</div>
            <div>
                ...
                <div style="font-size:12px;color:var(--muted,#5A5A5A);margin-top:2px;">
                    <i class="bi bi-envelope"></i> ${escHtml(alumno.correo)}
                </div>
                <div style="font-size:11px;color:#4A90B9;margin-top:4px;font-weight:600;">
                    <i class="bi bi-collection-play"></i> ${escHtml(curso.titulo)}
                </div>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:24px;">
            <div class="detalle-stat">
                <div class="detalle-stat-num">${pct}%</div>
                <div class="detalle-stat-label">Progreso</div>
            </div>
            <div class="detalle-stat">
                <div class="detalle-stat-num">${alumno.total_lecciones}</div>
                <div class="detalle-stat-label">Total lecciones</div>
            </div>
            <div class="detalle-stat">
                <div class="detalle-stat-num">${statusLabel}</div>
                <div class="detalle-stat-label">Estado</div>
            </div>
        </div>
        ...
        <!-- Módulos y lecciones -->
        <div style="font-family:'Syne',sans-serif;font-size:15px;font-weight:700;color:#1E4978;margin-bottom:14px;">
            <i class="bi bi-list-ul" style="margin-right:6px;"></i>Contenido del curso
        </div>
